    ...
    <>
      ...
      <div className="md:hidden fixed bottom-6 inset-x-0 z-[60] flex items-center justify-center pointer-events-none px-4">
        <div className="w-full max-w-md flex items-center justify-between gap-2 h-[64px] pointer-events-auto relative">
           ...
           <div
             className={`flex items-center justify-around h-full bg-neutral-800/40 backdrop-blur-2xl saturate-[180%] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.6)] rounded-full px-2 overflow-hidden transition-all duration-300 cubic-bezier-[0.32,0.72,0,1] ease-out flex-grow origin-left ${searchMode ? 'opacity-0 scale-95 pointer-events-none absolute w-[80%]' : 'opacity-100 scale-100 relative'}`}
           >
              <Link href="/" onClick={() => setSearchMode(false)} className={`flex flex-col items-center justify-center flex-1 h-full cursor-pointer gap-1 transition-all ${pathname === '/' ? 'text-white scale-105' : 'text-neutral-400 hover:text-neutral-300'}`}>
                 <Compass className="w-6 h-6" />
                 <span className="text-[10px] font-semibold tracking-wide">Discover</span>
              </Link>

              <Link href="/library" onClick={() => setSearchMode(false)} className={`flex flex-col items-center justify-center flex-1 h-full cursor-pointer gap-1 transition-all ${pathname === '/library' ? 'text-white scale-105' : 'text-neutral-400 hover:text-neutral-300'}`}>
                 <Library className="w-6 h-6" />
                 <span className="text-[10px] font-semibold tracking-wide">Library</span>
              </Link>

              <Link href="/playlists" onClick={() => setSearchMode(false)} className={`flex flex-col items-center justify-center flex-1 h-full cursor-pointer gap-1 transition-all ${pathname.includes('/playlists') ? 'text-white scale-105' : 'text-neutral-400 hover:text-neutral-300'}`}>
           ...
           <button
             ...
             className={`flex items-center justify-center h-full bg-neutral-800/40 backdrop-blur-2xl saturate-[180%] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.6)] rounded-full shrink-0 transition-all duration-300 cubic-bezier-[0.32,0.72,0,1] ease-out ${searchMode ? 'w-[64px] opacity-100 scale-100 relative' : 'opacity-0 scale-95 pointer-events-none absolute w-[64px]'}`}
           >
           ...
           >
              ...
              <input
                 ...
                 placeholder="Search..."
                 className="w-full bg-transparent border-none text-white text-base font-medium focus:outline-none px-3 placeholder-neutral-500"
