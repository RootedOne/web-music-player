...
export default function AlbumSongsPage() {
  ...
  const decodedAlbumName = albumName ? decodeURIComponent(albumName as string) : "";
  ...
  useEffect(() => {
    const fetchArtist = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Filter tracks for this specific album
          const albumTracks = (data.tracks || []).filter(
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (t: any) => t.album === decodedAlbumName
          );
    ...
    if (id && decodedAlbumName) fetchArtist();
  }, [id, decodedAlbumName]);
  ...
  return (
    <div className="relative min-h-screen bg-black text-white pb-36 font-sans">
      <nav className="sticky top-0 z-50 w-full flex items-center gap-4 px-4 py-4 md:py-6 bg-black/80 backdrop-blur-md border-b border-white/5 transition-all pt-safe">
        ...
        <h1 className="text-xl font-bold truncate">{decodedAlbumName}</h1>
      </nav>
      ...
      <div className="px-6 mx-auto max-w-7xl mt-6">
        ...
        {tracks.length === 0 && (
          <p className="text-gray-400 mt-8 text-center">No songs found in this album.</p>
        )}
      </div>
    </div>
  );
